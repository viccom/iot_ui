       ...
       (function register(){
           ...
           //姓名栏失去焦点
            $(".block .FirstName").blur(function(){
                var isfirstname = /^(?!_)(?!.*?_$)[a-zA-Z0-9_\u4e00-\u9fa5]+$/;
                if( $(this).val()==""|| $(this).val()=="请输入用户的姓")
                {
                    $(this).next().html("请输入用户的姓");
                }
                else if(!isfirstname.test($(".block .FirstName").val()))
                {
                    $(this).next().html("仅支持汉字、数字、字母、下划线，不能以下划线开头和结尾!");
                }
                else
                {
           ...
           //手机号栏失去焦点
            $(".block .phone").blur(function(){
                var isPhone=/^1[3|4|5|7|8][0-9]\d{4,8}$/i;//验证手机正则(输入前7位至11位)

                if( $(this).val()==""|| $(this).val()=="请输入用户手机号")
                {
                    $(this).next().html("请输入用户手机号");
                }
                else if($(this).val().length<11)
                {
                    $(this).next().html("手机号长度有误！");
                }
                else if(!isPhone.test($(".block .phone").val()))
                {
            ...
            $(".block .password").blur(function(){
                ...
                var repassword = $(".block .repassword").val();
                if(!reg.test($(".password").val()))
                {
                    $(this).next().html("格式有误，请输入3~20位的数字、字母或特殊字符！");
                }
                else if(password!=repassword)
                {
                    $(this).next().html("");
                    $(".block .repassword").next().html("两次密码输入不一致！");
                }
                else
                {
                    $(".block .repassword").next().html("");
                    $(this).next().html("");
                }
            });
            /*确认密码失去焦点*/
    ...
    function NewUserRegister(){
        var isEmail = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
        var isfirstname = /^(?!_)(?!.*?_$)[a-zA-Z0-9_\u4e00-\u9fa5]+$/;
        var isPhone = /^1[3|4|5|7|8][0-9]\d{4,8}$/i;
        var isValidPasswd=/^.{3,20}$/;
        ...
        if(email && FirstName && phone && password){
            if(!isEmail.test(email))
                {
                    $('#error_tips').html("邮箱格式错误!");
                    return;
                }
            if(!isfirstname.test(FirstName))
                {
                    $('#error_tips').html("姓名格式错误!");
                    return;
                }
            if(!isPhone.test(phone))
                {
                    $('#error_tips').html("手机号格式错误!");
                    return;
                }
            if(!isValidPasswd.test(password))
                {
                    $('#error_tips').html("密码规则错误!");
                    return;
                }
            if(password!=repassword)
                {
                    $('#error_tips').html("两次密码输入不一致！");
                    return;
                }
